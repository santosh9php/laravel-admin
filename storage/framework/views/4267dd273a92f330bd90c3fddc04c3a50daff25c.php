<div class="card" id="brand_add_model" >
   ...
   <div class="card-body" >
      <form action="<?php echo e(route('admin_post_brand_show')); ?>" method="post" enctype="multipart/form-data" onsubmit="return form_check()">
      <?php echo csrf_field(); ?>
            ...
            <div class="row"> 
               <div class="mb-6 col-md-9">
                     ...
                     <div class="row">
                      ...
                      <div class="row">
                        <div class="col-md-6">
                           <button type="submit" class="btn btn-primary">Submit</button> <a href="#" class="btn btn-danger" id="brand_hide_form">Hide Form</a>
               ...
               <div class="mb-6 col-md-3" style="overflow-y: scroll; height:400px;">
                  <div class="row">   
                     <div class="mb-12 col-md-12"><div id="ajax_brand_add_loader"></div></div>
                  </div>
                  <div class="row">
                        ...
                        <div id="parent_segment">
                           <?php
                              ...
                              if(old('category_id') !== 'null' and old('category_id') != ''){
                                 echo '<label class="form-label">Parent Segment</label>';
                                 $cat_parents = App\Http\Controllers\Admin\BrandController::getAllPrents(old('parent_id'));
                                 ...
                                 <ul id="myUL">
                                    <li>
                                       <?php if(count($brand->subBrand)): ?>
                                          <div class="caret open  <?php if(in_array($brand->id,$cat_parents) or in_array(old('parent_id'),$cat_parents)): ?> caret-down <?php endif; ?> ">

                                       <?php else: ?>
                                          <div class="uncaret open">
                                       <?php endif; ?>
                                       <input type="checkbox" id="html" class="radio-btn" name="parent_id" value="<?php echo e($brand->id); ?>" <?php if(old('parent_id') == $brand->id): ?> checked <?php endif; ?> >&nbsp;<?php echo e($brand->name); ?></div>
                                       
                                          <?php if(count($brand->subBrand)): ?>
                                          <ul class="nested  <?php if(in_array($brand->id,$cat_parents)): ?>   active <?php endif; ?>">
                                             <?php echo $__env->make('admin.subBrand.subBrand_addform',['subbrands' => $brand->subBrand,'cat_parents'=>$cat_parents], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
                                          </ul>
                                          <?php endif; ?>
...
<script language="javascript">
   ...
   function change_segment(category){
      ...
      $.ajax({
           type: 'POST',
           url:  '/admin/change_dropdown_segment',
           dataType: 'json',
           data:{'category':category,_token: '<?php echo e(csrf_token()); ?>'},
           beforeSend: function(){
               $('#ajax_brand_add_loader').show();
           },
           complete: function(){
               $('#ajax_brand_add_loader').hide();
           },
...
<?php /**PATH C:\xampp\htdocs\skmotor\resources\views/admin/brand_add.blade.php ENDPATH**/ ?>